...
# -------------------------
# Animation of the pendulum in the rotating frame
anim = @animate for i in 1:5:length(sol.t)
    current_time = sol.t[i]  # single Float64 time value at this frame
    ...
    plot(
        ...
        zlim = (-h1, L + h1),
        legend = false,
        title = @sprintf("Time = %6.2f s", current_time)
    )

    # Path traced so far by the bob
    plot!(x_vals[1:i], y_vals[1:i], z_vals[1:i], linewidth = 2)

    # Vertical support from ceiling down to pivot plane
    plot!([0, 0], [0, 0], [-h1, 0])

    # Arm from center to pivot (rotating about z-axis)
    plot!([0, w1 * cos(omega * current_time)],
          [0, w1 * sin(omega * current_time)],
          [0, 0])

    # Pendulum rod from pivot to bob
    plot!([w1 * cos(omega * current_time), x_vals[i]],
          [w1 * sin(omega * current_time), y_vals[i]],
          [0, z_vals[i]])

    # Bob as a point mass
    scatter!([x_vals[i]], [y_vals[i]], [z_vals[i]], markersize = 5)
end
...
# ╔═╡ 04990292-60dc-4519-80bb-8a5ef3260cee
md"""# 4. Slow vs Fast Rotation Comparison

In addition to the single-case plots above (team baseline), we solve the same equation of motion for two prescribed spin rates:

- **Slow**: Ω = Ω_slow
- **Fast**: Ω = Ω_fast

and compare the resulting motion, trajectories, and energies.
"""

# ╔═╡ cdb328c7-cd0b-4e63-8215-3ed1326702a1
begin
	Ω_slow = 2.0   # rad/s
	Ω_fast = 20.0  # rad/s
end

# ╔═╡ c7a1c100-9a71-4c82-bd23-0c5bd946fd31
begin
	# Parameterized ODE (avoid globals so we can solve multiple Ω values cleanly)
	function rotating_pendulum_param!(du, u, p, t)
		θ = u[1]
		θd = u[2]
		g, L, w1, Ω = p
		du[1] = θd
	...
	p_slow = (g, L, w1, Ω_slow)
	p_fast = (g, L, w1, Ω_fast)

	prob_slow = ODEProblem(rotating_pendulum_param!, u0, time, p_slow)
	prob_fast = ODEProblem(rotating_pendulum_param!, u0, time, p_fast)

	sol_slow = solve(prob_slow, saveat=0.01)
	sol_fast = solve(prob_fast, saveat=0.01)
end

# ╔═╡ 2db65765-6e02-4355-ac6c-d03e4e298858
begin
	# Inertial-frame bob coordinates for each solution
	function bob_xyz(sol, Ω; w1=w1, L=L)
		x = [(w1 + L*sin(sol[1,i])) * cos(Ω*sol.t[i]) for i in 1:length(sol.t)]
		y = [(w1 + L*sin(sol[1,i])) * sin(Ω*sol.t[i]) for i in 1:length(sol.t)]
		z = [-L*cos(sol[1,i]) for i in 1:length(sol.t)]
		return x, y, z
	end

	x_slow, y_slow, z_slow = bob_xyz(sol_slow, Ω_slow)
	x_fast, y_fast, z_fast = bob_xyz(sol_fast, Ω_fast)
end

# ╔═╡ c0cc26f5-cba8-4ca7-9237-ce98e18fc55d
begin
	# Compare angle and angular speed
	pθ = plot(sol_slow.t, sol_slow[1,:], xlabel="Time (s)", ylabel="θ (rad)",
		title="Angle vs Time (slow vs fast Ω)", label="slow Ω=$(Ω_slow)")
	plot!(pθ, sol_fast.t, sol_fast[1,:], label="fast Ω=$(Ω_fast)")

	pω = plot(sol_slow.t, sol_slow[2,:], xlabel="Time (s)", ylabel="θ̇ (rad/s)",
		title="Angular Velocity vs Time (slow vs fast Ω)", label="slow Ω=$(Ω_slow)")
	plot!(pω, sol_fast.t, sol_fast[2,:], label="fast Ω=$(Ω_fast)")

	plot(pθ, pω, layout=(2,1), size=(900,650))
end

# ╔═╡ d80f5de1-7755-4fe2-b791-a44275a9cb4a

begin
    # Trajectory comparison in x–z projection for slow vs fast angular velocity
    trajectory_plot = plot(
        x_slow, z_slow;
        xlabel = "x (m)",
        ylabel = "z (m)",
        title = "Trajectory (x–z) — slow vs fast Ω",
        linewidth = 2,
        aspect_ratio = :equal,
        label = "slow Ω = $(Ω_slow)"
    )
...
# ╔═╡ cc057935-e804-4269-8712-6da5aca92a03
begin
	# Energies for each case (same structure as the team plot, but with Ω separated from θ̇)
	function energies(sol, Ω; m=m, g=g, L=L, w1=w1, h1=h1)
		θ  = sol[1,:]
		θd = sol[2,:]
		r  = w1 .+ L .* sin.(θ)
		T  = 0.5*m .* (L^2 .* θd.^2 .+ (r .* Ω).^2)
		V  = m*g .* (h1 .- L .* cos.(θ))
		ℒ  = T .- V
		return T, V, ℒ
	end

	T_s, V_s, L_s = energies(sol_slow, Ω_slow)
	T_f, V_f, L_f = energies(sol_fast, Ω_fast)

	plt = plot(sol_slow.t, T_s, label="T (slow)", xlabel="Time (s)", ylabel="Energy (J)", linewidth=2,
		title="Energy vs Time — slow vs fast Ω")
	plot!(plt, sol_slow.t, V_s, label="V (slow)", linewidth=2)
	plot!(plt, sol_slow.t, L_s, label="ℒ=T−V (slow)", linewidth=2)

	plot!(plt, sol_fast.t, T_f, label="T (fast)", linestyle=:dash, linewidth=2)
	plot!(plt, sol_fast.t, V_f, label="V (fast)", linestyle=:dash, linewidth=2)
	plot!(plt, sol_fast.t, L_f, label="ℒ=T−V (fast)", linestyle=:dash, linewidth=2)

	plt
end

# ╔═╡ 2014c946-d0db-4702-bbb7-7dea8b4d93d7
begin
    # Build a 3D animation for a given solution and angular speed Ω
    function make_animation(sol, Ω, x, y, z; w1=w1, L=L, h1=h1, stride=5)
        @animate for i in 1:stride:length(sol.t)
            current_time = sol.t[i]
            plot(xlim=(-w1-L, w1+L), ylim=(-w1-L, w1+L), zlim=(-h1, L+h1),
                 legend=false,
                 title=@sprintf("Spinning Pendulum (Ω = %5.2f rad/s, t = %6.2f s)", Ω, current_time))

            plot!(x[1:i], y[1:i], z[1:i], linewidth=2)
            plot!([0,0], [0,0], [-h1,0], linewidth=2)

            plot!([0, w1*cos(Ω*current_time)], [0, w1*sin(Ω*current_time)], [0, 0], linewidth=3)
            plot!([w1*cos(Ω*current_time), x[i]], [w1*sin(Ω*current_time), y[i]], [0, z[i]], linewidth=3)
            scatter!([x[i]], [y[i]], [z[i]], markersize=5)
        end
    end

    anim_slow = make_animation(sol_slow, Ω_slow, x_slow, y_slow, z_slow)
    anim_fast = make_animation(sol_fast, Ω_fast, x_fast, y_fast, z_fast)
end

# ╔═╡ afc4feef-ba7b-4964-865e-0272a6b1cd06
begin
	gif(anim_slow, "rotating_pendulum_slow.gif", fps=20)
end

# ╔═╡ 65187603-f4cf-43eb-af4c-d07129bffb7e
begin
	gif(anim_fast, "rotating_pendulum_fast.gif", fps=20)
end
