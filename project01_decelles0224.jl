...
# ╔═╡ 5f353297-62e7-4154-9a01-275b9eecf0e3
using Symbolics          # symbolic math, derivatives, simplification

# ╔═╡ 9d1faa77-1966-4e69-9695-178f3d28f628
using DifferentialEquations  # solving the pendulum ODE

# ╔═╡ 4aa49339-eb96-4ed8-9c3a-1ae7d2a05224
...
# ╔═╡ 6247f4b1-e3fa-4ca5-9833-fdd0f1083604
begin
...
# ╔═╡ 12632565-594f-4517-a949-530857ac1bb6
begin
	...
	pendulum_ode!(time_derivative_state, state, parameters, current_time) = begin
	    ...
	    time_derivative_state[2] = -(gravity_value / length_value) * sin(angle_value)
	end
	
	pendulum_parameters = Dict(:gravity => 9.81, :length => 1.0)
	initial_state = [0.7, 0.0]             # 0.7 rad ≈ 40°, released from rest
	time_span = (0.0, 10.0)                # simulate for 10 seconds
	
end

# ╔═╡ 7ece0669-59cf-4f35-8292-1ee40b14ca85
begin
	
	# --- Solve the ODE using DifferentialEquations.jl ---
	pendulum_problem = ODEProblem(
	    pendulum_ode!,
	    initial_state,
	    time_span,
	    pendulum_parameters,
...
# ╔═╡ b1c47749-5642-4682-81de-97010c6c35ca
begin
	
	# --- Sample the solution to prepare data for plotting and animation ---
	time_samples = range(time_span[1], time_span[2], length = 300)
	
	angle_samples = [pendulum_solution(t)[1] for t in time_samples]
	angular_velocity_samples = [pendulum_solution(t)[2] for t in time_samples]
	
	length_value = pendulum_parameters[:length]
	pendulum_x = length_value .* sin.(angle_samples)
	pendulum_y = -length_value .* cos.(angle_samples)
	
end

# ╔═╡ 36003555-e6c5-480d-87f3-318eacecfa7a
begin
	
	# --- Create an animated GIF with angle, angular speed, and pendulum motion ---
	animation_file = @gif for index in eachindex(time_samples)
	    angle_plot = plot(time_samples[1:index], angle_samples[1:index],
	    ...
	    speed_plot = plot(time_samples[1:index], angular_velocity_samples[1:index],
	        xlabel = "time (s)", ylabel = "angular speed θ̇ (rad/s)",
	    ...
	    plot!(pendulum_plot, [0.0, pendulum_x[index]], [0.0, pendulum_y[index]], lw = 3)
...
manifest_format = "2.0"
project_hash = "b6c1588737cad02c1aeb3b3a47491b2574a98ee6"
...
git-tree-sha1 = "d7c3e8e7a24df51c377419b31fb288df110f117e"
